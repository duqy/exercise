...
public:
    // 构造函数
    // 直接调用基类 Employee 的构造函数，完成相同部分属性的构建211 6.4 工资程序成长记：类与对象 │
...
public:
    ...
    int Read()
    {
        ...
        if(in.is_open()) // 判断是否成功打开
        {
            ...
            while(true)
            {
                ...
                if(!in)
                    break; // 跳出读取循环
				// 根据读取的员工级别，分别创建不同的员工对象，213 6.4 工资程序成长记：类与对象 │
    ...
    int Input()
    {
		...
		// 又因为 i 在 for 循环之后还需要用到，所以定义在 for 循环之前214 6.4 工资程序成长记：类与对象 │
        int i = m_nCount;
        for(; i < MAX; ++i) // 初始化语句留空
        {
			// 利用 for 循环逐个输入
